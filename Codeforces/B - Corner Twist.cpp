...
void solve() {
    ...
    */
    ...
    int grid3[n][m], grid4[n][m];
    for(int i = 0; i < n; i++ ) {
        for(int j = 0; j < m; j++ ) {
            grid3[i][j] = grid1[i][j] - '0';
        }
    }
    for(int i = 0; i < n; i++ ) {
        for(int j = 0; j < m; j++ ) {
            grid4[i][j] = grid2[i][j] - '0';
        }
    }
    for(int i = 0; i < n - 1; i++ ) {
        for(int j = 0; j < m - 1; j++ ) {
             while(grid3[i][j] != grid4[i][j]) {
                convert(grid3[i][j], 1);
                convert(grid3[i + 1][j], 2);
                convert(grid3[i][j + 1], 2);
                convert(grid3[i + 1][j + 1], 1);
             }
        }
    }
    bool check = 1;
